...
let h = 500;
let padding = 50;
let colStr = ["whitesmoke","snow","ivory","lavender","powderblue","mistyrose","lightblue","cyan","plum","lightcoral","mediumslateblue","orchid","aqua","mediumspringgreen","darkturquoise","firebrick","indianred","deeppink","lightpink","crimson","gold"]
...
function add(){
  addDatapoints(1);
  dataGroups.select("rect").attr("fill","dimgray")
  ...
  theSituation.transition().duration(1000);
...
function removeAndAdd(){
  removeAndAddDatapoints(Math.floor(Math.random()*data.length),Math.floor(Math.random()*data.length));
  dataGroups.select("rect").attr("fill","dimgray")
  console.log("new data", data)
  allNames = data.map(function(d){return d.key});
  xScale.domain(allNames);
...
function colorData(){
  let colorScale = d3.scaleLinear().domain([0,10,30]).range([colStr[Math.floor(Math.random()*colStr.length)],colStr[Math.floor(Math.random()*colStr.length)],colStr[Math.floor(Math.random()*colStr.length)]])

  theSituation = graphGroup.selectAll(".datapoint").data(data,function(d){return d.key});
  theSituation.transition().duration(100)
  .attr("transform", function(d, i){
    return "translate("+ xScale(d.key)+ "," + (h - padding) + ")"
  });
...
document.getElementById("buttonF").addEventListener("click",colorData)
